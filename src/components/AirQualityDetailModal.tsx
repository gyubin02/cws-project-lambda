...
const thresholds = {
  pm10: [
    ...
    { max: 80, grade: 'MODERATE', label: '보통', icon: AlertCircle, color: 'text-secondary' },
    { max: 150, grade: 'BAD', label: '나쁨', icon: AlertCircle, color: 'text-warning' },
    { max: Infinity, grade: 'VERY_BAD', label: '매우 나쁨', icon: XCircle, color: 'text-destructive' },
  ],
  pm25: [
    { max: 15, grade: 'GOOD', label: '좋음', icon: CheckCircle, color: 'text-success' },
    { max: 35, grade: 'MODERATE', label: '보통', icon: AlertCircle, color: 'text-secondary' },
...
export function AirQualityDetailModal({ data, open, onOpenChange }: AirQualityDetailModalProps) {
  ...
  const guidance = data.grade ? recommendations[data.grade] : null;
        ...
        <div className="space-y-6 mt-4">
          ...
          {isError ? (
            <div className="flex items-center gap-2 text-muted-foreground p-4 rounded-lg bg-muted/50">
              ...
              <span>{data.note || '공기질 정보를 가져올 수 없습니다.'}</span>
            </div>
          ) : hasData && guidance ? (
            <>
              <div className="p-4 rounded-lg bg-muted/50">
                <h3 className="font-semibold mb-2">{guidance.title}</h3>
                <p className="text-sm text-foreground">{data.note || '현재 공기질 상태입니다.'}</p>
              </div>
