/**
 * Briefing 컨트롤러: 통합 외출 브리핑 조회.
 * IMPLEMENTATION STATUS: OK (validation + location resolution + service orchestration)
 */
...
export async function getBriefingController(query: {
  user_id?: string;
  from?: string;
  to?: string;
  time?: string;
  ...
  try {
    ...
    if (originResult.error) {
      return {
        statusCode: originResult.error.status,
        body: {
    ...
    if (destinationResult.error) {
      return {
        statusCode: destinationResult.error.status,
        body: {
    ...
    return { statusCode: 200, body: { data: briefing, meta } };
  } catch (error) {
